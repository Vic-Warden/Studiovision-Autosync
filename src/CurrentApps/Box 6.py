...
SOURCE_DIR  = Path(r"\\studiovision\Studiov2000\BIOMETRIES")  # Acquisition drop folder
ORPHAN_DIR  = Path(r"C:\Users\Box-6\Desktop\Images_Oubliées")  # Unmatched files destination
DEST_PHOTOS = Path(r"\\studiovision\Studiov2000-OM\PHOTOS")  # Network photo archive root
...
def _launch_studio_vision() -> None:
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if not still_alive:
                break
            log.debug(f"Waiting for PIDs to fully exit: {still_alive}")
