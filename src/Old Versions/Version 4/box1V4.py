"""
box1V4.py
Image Router — Version 4, Box 1 (standard imaging device build).

This variant targets the first imaging station ("Box 1") and adds all Version 4
...
Architecture — producer/consumer with burst debounce:
  ...
  pystray runs in the main thread (Windows requirement); the observer loop and
  worker run in daemon threads.
...
def main() -> None:
    ...
    _icon.run()  # Blocks main thread — required by pystray on Windows.
