"""
studiovision_monitor_V6.py
Medical imaging router — Version 6

Removed pyodbc/SQL/Access DB layer. Record insertion uses win32com GUI automation
to write directly into the SFDoc subform of the active Access form.
The script launches Studio Vision via subprocess (blocking) while watchdog threads
run in the background. When Studio Vision closes, all threads stop and the script exits.
...
# Path to the folder where the acquisition system drops new image files.
SOURCE_DIR  = Path(r"??")

# Destination for files that could not be matched to a patient.
ORPHAN_DIR  = Path(r"??")

# Root of the network photo archive (files are moved here before GUI insert).
...
# File extensions to watch and their human-readable description for the log.
...
FILE_LOCK_MAX_ATTEMPTS: int = 15   # maximum file-lock retry attempts
PATIENT_POLL_INTERVAL:  int = 3    # seconds between "is a patient open?" checks
PATIENT_WAIT_TIMEOUT:   int = 900  # seconds to wait for a patient before orphaning
...
# Name of the subform that receives the new document path.
SFDOC_SUBFORM_NAME = "SFDoc"

# Access control-type constant for subforms.
_AC_SUBFORM = 112

# Delay (seconds) between file move and GUI insert to let the OS settle.
_GUI_PRE_INSERT_DELAY = 0.3

# Delay (seconds) between insert and Requery/MoveLast.
_UI_POST_INSERT_DELAY = 0.5
...
def build_patient_relative_path(patient_code: str, last_name: str, first_name: str) -> str:
    """
    Computes the Studio Vision relative folder path from patient data.

    Format: [First 2 digits of code].000\\[Full code][First 3 of last].[First 3 of first]
...
def resolve_patient_folder(patient: dict) -> Path | None:
    """
    Resolves and, if necessary, creates the absolute patient folder on disk.
    Returns None if the path cannot be determined or created.
    """
    try:
...
def get_active_patient() -> dict | None:
    """
    Reads the patient code, last name, and first name from the active Access form.
    Returns a dict with keys 'code', 'nom', 'prenom', or None if unavailable.
    """
...
# Access COM helpers — finding the SFDoc subform
# ---------------------------------------------------------------------------

def _find_sfdoc(form) -> object | None:
    """
    Recursively searches the form's control tree for the SFDoc subform.
    Returns the subform's Form object, or None if not found.
    """
    for i in range(form.Controls.Count):
        ctrl = form.Controls(i)
        try:
...
def gui_insert_document(patient: dict, relative_path: str) -> bool:
    """
    Inserts a new document record into the SFDoc subform via win32com GUI automation.
    ...
    try:
        ...
        # Verify the correct patient is still open before inserting.
        ...
        # Move to a new (blank) record at the end of the subform.
        sfdoc.Recordset.AddNew()
        log.debug("AddNew() called on SFDoc recordset.")

        # Write the relative path into every relevant field.
        # Adjust field names below to match the actual SFDoc column names.
        try:
            sfdoc.Recordset.Fields("Photo externe").Value = relative_path
        except Exception:
            pass  # Field name may differ; the DoCmd approach below is the fallback.
        ...
        # Short pause to allow Access / Jet to finalise the write.
        time.sleep(_UI_POST_INSERT_DELAY)

        # Requery the subform so the record appears in the view.
        try:
            sfdoc.Requery()
...
def worker(file_queue: queue.Queue) -> None:
    """
    Consumes files from file_queue.
    For each file: waits for readability, polls for an open patient, resolves the
    patient folder, moves the file to the network share, then inserts via GUI.
    """
    ...
    try:
        while True:
            ...
            # Resolve patient folder on the network share
            ...
            if gui_insert_document(patient, relative_path):
                ...
                burst_count      += 1
                log.info(f"Record inserted into SFDoc: '{dest.name}' → {relative_path}")
                log_doctor(
                ...
                log.error(
                    f"GUI insertion FAILED for '{dest.name}' (patient {patient['code']}). "
                    f"File is at: {dest}. Manual record entry required."
                )
                _notify("Insert error", f"'{dest.name}' moved but not inserted — see log.")
                log_doctor(
                    f"ERROR: '{dest.name}' was moved to {dest} but could NOT be inserted "
                    f"into the Access form for patient {patient['nom']} {patient['prenom']} "
...
def _launch_studio_vision() -> None:
    """Launches Studio Vision as a blocking child process. Sets _stop_event on exit."""
    log.info(f"Launching Studio Vision: {' '.join(STUDIO_VISION_CMD)}")
    try:
        result = subprocess.run(STUDIO_VISION_CMD, check=False)
        log.info(
            f"Studio Vision exited (return code: {result.returncode}). "
            "Initiating shutdown."
        )
        ...
        log.error(f"Studio Vision process error: {exc}. Shutting down.")
    finally:
        _stop_event.set()
        if _icon is not None:
            try:
...
def main() -> None:
    ...
    if parent_name == "explorer.exe":
        ...
        if sv_running:
            ctypes.windll.user32.MessageBoxW(
                0,
                "To restart the image router, please close Studio Vision completely and relaunch it.",
                "Image Router",
                0x30,
            )
            sys.exit(0)
    ...
    # Launch Studio Vision (blocking subprocess); sets _stop_event on exit
    ...
    _icon = pystray.Icon(name=BOX_NAME, icon=_make_icon(_COLOR_READY), title=BOX_NAME, menu=menu)
