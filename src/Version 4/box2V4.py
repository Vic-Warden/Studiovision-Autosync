"""
box2V4.py
Image Router — Version 4, Box 2 (Nidek OCT device build).

Extends box1V4.py with specialised handling for the Nidek OCT scanner, which
...
All Version 4 improvements (PollingObserver, network share wait, auto-reconnect,
targeted SFDoc Requery, dirty-state guard, retry loop, patient-code guard,
burst debounce, log in ~/studiovision/, system tray icon) are also included.
...
#  Worker thread (consumer) — Nidek deduplication preserved
...
def worker(file_queue: queue.Queue) -> None:
    ...
    try:
        while True:
            ...
            # First file of a new burst — notify user and switch icon to green.
