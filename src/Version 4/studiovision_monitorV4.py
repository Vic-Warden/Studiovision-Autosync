"""
studiovision_monitorV4.py
Image Router — Version 4, latest feature build for modern Windows.

Improvements over earlier versions:
  1. PollingObserver — reliable on network shares.
  2. Network share wait at startup.
  3. Auto-reconnect loop.
  4. Targeted parent Refresh() + SFDoc-only Requery().
  5. Dirty-state guard.
  6. Requery retry loop.
  7. Patient-code guard.
  8. Log in ~/studiovision/.
  9. System tray icon (pystray + Pillow) — blue when idle, green when active.
     Right-click menu: status (read-only), open logs, quit.
     Windows toast notifications at burst start and burst end.

Architecture — producer/consumer, single worker thread:
  Producer: Watchdog PollingObserver on SOURCE_DIR (recursive).
  Consumer: worker() thread with burst debounce (1.5 s idle).
  pystray runs in the main thread (Windows requirement).

Dependencies: watchdog, pyodbc, pywin32, pythoncom, pystray, Pillow
"""
...
_ICON_SIZE     = 64
_COLOR_READY   = (30, 144, 255)   # dodger blue — idle
_COLOR_ACTIVE  = (50, 205, 50)    # lime green  — processing
